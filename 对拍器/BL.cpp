...
struct Seg {
    ...
    void calc(Node &u, int add, int mul) {
        ...
        int mul2 = mul * mul;
        int add2 = add * add;
        u.sum2 = u.sum2 * mul2 + 2 * lvl * u.sum1 * add * mul2 + lvl * add2 * mul2;
...
void solve() {
    ...
    while (m --) {
        ...
        // for (int i = 1; i <= n; i ++) cout << sg.query(1, i, i).sum1 << " \n"[i == n];
        // for (int i = 1; i <= n; i ++) cout << sg.query(1, i, i).sum2 << " \n"[i == n];
        // cout << "---------------" << '\n';
    }
}
